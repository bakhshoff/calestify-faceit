...
# ============================================================
# AYARLAR - Bunları dəyişdirin
# ============================================================
BOT_TOKEN = "azjBRqw6S76OQ4ArCh8dYcMXJ0OYyL2y"
MATCHMAKING_KANAL_ID = 0       # #1500580102151340132
NETICE_KANAL_ID = 0            # #1500580157600170125
MODERATOR_ROL_ID = 0           # #1487596991776034856
# ============================================================
...
queue = []          # sıradakı oyunçular
aktiv_matchlər = {} # match_id -> match məlumatları
...
async def queue_cix_handler(interaction: discord.Interaction):
    ...
    onceki_uzunluq = len(queue)
    queue = [o for o in queue if o["discord_id"] != discord_id]

    if len(queue) < onceki_uzunluq:
...
async def match_baslat(kanal):
    ...
    # ELO-ya görə ən yüksək oyunçu kapitan olur
    ...
    embed = discord.Embed(
        title=f"⚔️ Match Başladı! | {match_id}",
        color=0x00FF88
    )
    embed.add_field(name="🗺️ Xəritə", value=xerite, inline=False)
    embed.add_field(
        name="🔵 Komanda A",
        value=oyuncu_siyahi(komanda_a, kapitan_a),
        inline=True
    )
    embed.add_field(
        name="🔴 Komanda B",
        value=oyuncu_siyahi(komanda_b, kapitan_b),
        inline=True
    )
    embed.add_field(
        name="👑 Kapitanlar",
...
async def netice(
    ...
    məğlub_komanda = match["komanda_b"] if qalib == "A" else match["komanda_a"]
    ...
    embed = discord.Embed(
        title=f"🏁 Match Nəticəsi | {match_id}",
        color=0x00FF88 if qalib == "A" else 0xFF4444
    )
    embed.add_field(name="🗺️ Xəritə", value=match["xerite"], inline=True)
    embed.add_field(name="📊 Skor", value=skor, inline=True)
    embed.add_field(name=f"🏆 Qalib — Komanda {qalib}", value="", inline=False)
    embed.add_field(
        name="🔵 Komanda A" + (" 🏆" if qalib == "A" else ""),
        value=siyahi_yaz(match["komanda_a"], qalib == "A"),
        inline=False
    )
    embed.add_field(
        name="🔴 Komanda B" + (" 🏆" if qalib == "B" else ""),
        value=siyahi_yaz(match["komanda_b"], qalib == "B"),
        inline=False
    )
...
async def aktiv_matchler(interaction: discord.Interaction):
    ...
    for mid, m in aktiv_matchlər.items():
        embed.add_field(
            name=f"Match {mid}",
            value=f"Xəritə: {m['xerite']}\nBashlama: {m['bashlama'][:16]}",
            inline=False
        )
    await interaction.response.send_message(embed=embed)
...
async def on_ready():
    ...
    # Slash komandaları sinxronlaşdır
    await bot.tree.sync()
    print("✅ Slash komandalar sinxronlaşdı!")

    # Matchmaking kanalında sabit mesaj göndər
    kanal = bot.get_channel(MATCHMAKING_KANAL_ID)
    if kanal:
        # Köhnə bot mesajlarını sil
        async for msg in kanal.history(limit=50):
            if msg.author == bot.user:
                await msg.delete()

        # Yeni sabit mesaj göndər
        await kanal.send(embed=queue_embed_yarat(), view=QueueView())
        print(f"✅ Matchmaking mesajı #{kanal.name} kanalına göndərildi!")

    bot.add_view(QueueView())  # Yenidən başladıqda düymələr aktiv qalsın
